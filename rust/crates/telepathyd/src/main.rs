...
const API_TOKEN_COMPARISON_CONTEXT: &[u8] = b"telepathyd/api-token-digest-compare/v1";
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::AmbiguousPostOpen(error) => write!(
                formatter,
                "interaction journal append may have written bytes before failing; restart telepathyd to reconcile before recording or retrying interactions: {error}"
            ),
...
async fn main() -> anyhow::Result<()> {
    ...
    let pending_path =
        PathBuf::from(std::env::var("TELEPATHY_PENDING").unwrap_or_else(|_| "pending.json".into()));
    let bind = std::env::var("TELEPATHY_BIND").unwrap_or_else(|_| "127.0.0.1".into());
    let lanes_path_buf = PathBuf::from(&lanes_path);
    let api_token_configured = std::env::var("TELEPATHY_TOKEN")
        .map(|token| !token.is_empty())
        .unwrap_or(false);
    if api_token_required(&bind, api_token_configured) {
        return Err(anyhow::anyhow!(
            "TELEPATHY_TOKEN is required when TELEPATHY_BIND is non-loopback"
        ));
    }
    if relay_credentials_required(&bind, api_token_configured, secrets.is_empty()) {
        return Err(anyhow::anyhow!(
            "TELEPATHY_RELAY_SECRETS is required when TELEPATHY_TOKEN is set or TELEPATHY_BIND is non-loopback"
        ));
    }
    let tls_cert = std::env::var("TELEPATHY_TLS_CERT").ok();
    let tls_key = std::env::var("TELEPATHY_TLS_KEY").ok();
    if tls_cert.is_some() != tls_key.is_some() {
        return Err(anyhow::anyhow!(
            "TELEPATHY_TLS_CERT and TELEPATHY_TLS_KEY must be configured together"
        ));
    }
    ...
    {
        return Err(anyhow::anyhow!(
            "TELEPATHY_TLS_CERT and TELEPATHY_TLS_KEY are required for authenticated non-loopback endpoints"
        ));
    }
    ...
    if let (Some(cert), Some(key)) = (tls_cert, tls_key) {
        ...
        println!("telepathyd lane API on https://{addr}");
...
fn parse_bind_addr(bind: &str, port: u16) -> anyhow::Result<SocketAddr> {
    ...
    address
        .parse()
        .map_err(|error| anyhow::anyhow!("invalid TELEPATHY_BIND {bind:?}: {error}"))
}
...
fn parse_interaction_snapshot(path: &Path) -> anyhow::Result<Vec<InteractionEntry>> {
    ...
    if version != Some(INTERACTION_LEDGER_VERSION.into()) {
        return Err(anyhow::anyhow!(
            "interaction ledger {} uses an unsupported format; telepathyd now requires version {} (hard cutover)",
            path.display(),
    ...
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::PersistenceUncertain => write!(
                formatter,
                "lane persistence is uncertain after a prior post-rename failure; restart telepathyd to reconcile before changing lanes"
            ),
...
fn require_interaction_persistence_available(state: &AppState) -> anyhow::Result<()> {
    ...
    {
        Err(anyhow::anyhow!(
            "interaction journal persistence is uncertain after a prior append failure that may have written bytes; restart telepathyd to reconcile before recording or retrying interactions"
        ))
    } else {
        Ok(())
    }
...
async fn api_auth(request: Request, next: Next) -> Response {
    let expected = std::env::var("TELEPATHY_TOKEN").unwrap_or_default();
...
async fn post_message(
    ...
    let lane_id = match body.get("lane_id") {
        None => "telepathy:direct".to_string(),
    ...
    let reply = match &action {
        ...
        telepathy_lanes::MetaAction::Unknown => {
            ...
            let reply = telepathy_lanes::execute(&mut reg, action.clone(), std::path::Path::new(&notes_path));
                ...
                let reply =
                    match telepathy_steering::run(&provider, &mut proposed, &utterance).await {
                        Ok(reply) => reply,
...
mod tests {
    ...
    fn temp_path(label: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("telepathyd-main-{label}-{nonce}.json"))
    }
    ...
    fn temp_directory(label: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("telepathyd-main-{label}-{nonce}"))
    }
    ...
    fn full_lane_registry() -> LaneRegistry {
        ...
        for index in 1..MAX_LANE_COUNT {
            registry
                .create(&format!("{index:03}-{}", "x".repeat(109)))
                .unwrap();
        }
    ...
    fn state_title_enrichment_safely_truncates_without_touching_persisted_lane_metadata() {
        ...
        enrich_state_titles(&mut body, vec![("telepathy:direct".into(), huge_title)]);
    ...
    async fn lane_capacity_is_a_permanent_api_error_without_mutation_or_persistence() {
        ...
        *state.lane_save_fault.lock().unwrap() = Some(LaneSaveFault::PreRename);
        ...
        assert_eq!(
            ...
            telepathy_lanes::LANE_CAPACITY_ERROR_MESSAGE
        );
        assert_eq!(*state.reg.lock().await, before);
        assert_eq!(state.registry_revision.load(Ordering::SeqCst), 10);
        assert!(!path.exists());
    ...
    async fn touching_unknown_lane_returns_404_without_mutating_or_persisting() {
        ...
        *state.lane_save_fault.lock().unwrap() = Some(LaneSaveFault::PreRename);
        ...
        assert_eq!(body["error"], "unknown lane telepathy:missing");
        assert_eq!(*state.reg.lock().await, before);
        assert_eq!(state.registry_revision.load(Ordering::SeqCst), 10);
        assert!(!path.exists());
        assert!(state.lane_save_fault.lock().unwrap().is_some());
    ...
    async fn touching_when_persistence_is_uncertain_returns_503_without_mutating() {
        ...
        *state.lane_save_fault.lock().unwrap() = Some(LaneSaveFault::PreRename);
        ...
        let response = touch(
            State(state.clone()),
            Json(serde_json::json!({ "id": "telepathy:direct" })),
        )
        .await;
    ...
    fn api_token_uses_fixed_digests_and_preserves_header_bearer_semantics() {
        ...
        headers.insert("x-telepathy-token", HeaderValue::from_static("secret"));
        ...
        headers.insert("x-telepathy-token", HeaderValue::from_static("wrong"));
        assert!(!api_token_matches(&headers, "secret"));
    ...
    fn relative_registry_revision_path_uses_current_directory() {
        let name = format!(
            ".telepathyd-relative-revision-{}-{}.json",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
    ...
    async fn unknown_meta_without_model_at_revision_ceiling_remains_read_only() {
        ...
        environment.set("TELEPATHY_META_MODEL", "");
    ...
    async fn model_meta_provider_failures_are_sanitized_and_never_merge_mutations() {
        ...
        let oversized = telepathy_steering::MAX_PROVIDER_RESPONSE_BYTES + 1;
    ...
    async fn model_meta_transport_failure_is_a_sanitized_502_without_mutation() {
        ...
        environment.set("TELEPATHY_META_MODEL", "test-model");
        environment.set("OPENAI_API_KEY", "test-key");
        environment.set("TELEPATHY_META_BASE_URL", format!("http://{address}"));
    ...
    async fn model_meta_enforces_exact_reply_text_boundaries_before_serializing() {
        ...
        environment.set("TELEPATHY_META_MODEL", "test-model");
        environment.set("OPENAI_API_KEY", "test-key");
        let exact_ascii = "a".repeat(telepathy_steering::MAX_REPLY_TEXT_BYTES);
        let exact_multibyte = "🦀".repeat(telepathy_steering::MAX_REPLY_TEXT_BYTES / "🦀".len());
        assert_eq!(
            exact_multibyte.len(),
            telepathy_steering::MAX_REPLY_TEXT_BYTES
        );
        let too_large = "a".repeat(telepathy_steering::MAX_REPLY_TEXT_BYTES + 1);
        let (server_address, server) = spawn_meta_test_server_with_responses(vec![
            openai_json_response(serde_json::json!({
        ...
        for (label, expected) in [
            ("exact-ascii", telepathy_steering::MAX_REPLY_TEXT_BYTES),
            ("exact-multibyte", telepathy_steering::MAX_REPLY_TEXT_BYTES),
        ] {
    ...
    async fn meta_lane_capacity_returns_a_stable_permanent_error_without_mutation() {
        ...
        *state.lane_save_fault.lock().unwrap() = Some(LaneSaveFault::PreRename);
        ...
        let body = response_json(response).await;
        assert!(body.get("reply").is_none());
        assert_eq!(body["error"], telepathy_lanes::LANE_CAPACITY_ERROR_MESSAGE);
        assert_eq!(*state.reg.lock().await, before);
        assert!(!path.exists());
        assert!(state.lane_save_fault.lock().unwrap().is_some());
    }
    ...
    let relay = Arc::new(RelayState::default());
        ...
        let correlated = relay
            .queue_gateway_delivery("telepathy:direct", "receipt-owned reply", Some("tp-owned"))
            .unwrap();
        let unspoken = relay
            .queue_delivery("telepathy:direct", "later generic reply")
            .unwrap();
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let remaining = relay.pending_for("telepathy:direct");
        assert_eq!(
            remaining
                .iter()
    ...
    async fn delivery_head_is_bounded_and_tracks_the_durable_high_water_mark() {
        ...
        state
            .relay
            .queue_delivery("telepathy:other", &large)
            .unwrap();
        state
            .relay
            .queue_delivery("telepathy:other", &large)
        ...
        assert_eq!(state.relay.pending_count("telepathy:other"), 2);

        state
            .relay
            .deliveries_after(0, true, Some("telepathy:other"), None, None)
            .unwrap();
    ...
    async fn post_message_rejects_present_malformed_lane_without_registering_or_pushing() {
        ...
        assert!(state.relay.pending_for("telepathy:direct").is_empty());
    ...
    async fn post_message_rejects_permanent_oversize_as_413_without_retry_mutation() {
        ...
        for text in [envelope_overhead_text, multibyte_text] {
            let event = relay::message_event("telepathy:direct", "direct", &text, 0);
            assert!(
    ...
    let relay = Arc::new(RelayState::default());
        ...
        let response = get_delivery(
            ...
            axum::extract::Query(DeliveryQuery {
                after: 0,
                consume: true,
                lane_id: Some("telepathy:direct".into()),
                reply_to: None,
        ...
        .await;
        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
        assert_eq!(relay.pending_count("telepathy:direct"), 2);
    ...
    let relay = Arc::new(RelayState::default());
        ...
        let direct_seq = relay
            ...
            .unwrap();
        let other_seq = relay
            .queue_delivery("telepathy:other", "other reply")
            .unwrap();
        let snapshot_before = fs::read_to_string(&pending_path).unwrap();
        let state = Arc::new(AppState {
            reg: Arc::new(Mutex::new(LaneRegistry::default_direct())),
        ...
        for (lane_id, expected_status) in [
            ("", StatusCode::BAD_REQUEST),
            (" \t\n", StatusCode::BAD_REQUEST),
            ("telepathy:missing", StatusCode::NOT_FOUND),
        ] {
            let response = get_delivery(
                State(state.clone()),
                axum::extract::Query(DeliveryQuery {
            ...
            assert_eq!(relay.pending_count("telepathy:direct"), 1);
            assert_eq!(relay.pending_count("telepathy:other"), 1);
            assert_eq!(fs::read_to_string(&pending_path).unwrap(), snapshot_before);
        }
        ...
        let response = get_delivery(
            ...
            axum::extract::Query(DeliveryQuery {
                after: 0,
                consume: false,
                lane_id: Some("telepathy:missing".into()),
                reply_to: None,
                through_seq: None,
            }),
        )
        ...
        .await;
        assert_eq!(response.status(), StatusCode::OK);
        assert_eq!(relay.pending_count("telepathy:direct"), 0);
        assert_eq!(relay.pending_count("telepathy:other"), 1);
    ...
    async fn consuming_delivery_requires_exact_nonblank_reply_to_without_mutation() {
        ...
        relay.set_persist_path(&pending_path);
        let generic = relay
            .queue_delivery("telepathy:direct", "generic update")
            .unwrap();
        let owned = relay
            .queue_gateway_delivery("telepathy:direct", "owned reply", Some("tp-owned"))
            .unwrap();
        let other = relay
            .queue_gateway_delivery("telepathy:direct", "other reply", Some("tp-other"))
            .unwrap();
        let snapshot_before = fs::read(&pending_path).unwrap();
        ...
        for reply_to in [None, Some("".to_string()), Some(" \t\n".to_string())] {
            ...
            .await;
            assert_eq!(response.status(), StatusCode::BAD_REQUEST);
            assert_eq!(state.relay.pending_count("telepathy:direct"), 3);
            assert_eq!(fs::read(&pending_path).unwrap(), snapshot_before);
        }
        ...
        .await;
        assert_eq!(response.status(), StatusCode::OK);
        assert_eq!(state.relay.pending_count("telepathy:direct"), 2);
        let remaining = state.relay.pending_for("telepathy:direct");
