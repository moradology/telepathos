...
use telepathos_lanes::{MAX_ENRICHED_LANE_TITLE_CODEPOINTS, MAX_ENRICHED_LANE_TITLE_UTF8_BYTES};
use telepathos_proto::{MAX_OPAQUE_ID_BYTES, MAX_OPAQUE_ID_LENGTH};
    ...
    impl TestDatabase {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "telepathos-hermes-search-{}-{id}.sqlite",
                std::process::id()
            ));
