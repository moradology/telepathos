...
//!
//! Config:
//!   TELEPATHOS_NTFY_URL    e.g. https://ntfy.sh/telepathos-<secret-suffix>
//!   TELEPATHOS_NTFY_TOKEN  optional access token (self-hosted auth)
...
impl NtfyPush {
    ...
    async fn post(&self, lane_name: &str, content: &str) {
        let mut req = self
            .client
            .post(&self.url)
            .header("Title", format!("Telepathos · {lane_name}"))
            .header("Tags", "ear")
