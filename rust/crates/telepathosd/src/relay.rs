...
use telepathos_lanes::{is_valid_lane_id, LaneRegistry};
...
mod queue_tests {
    ...
    fn reload_advances_sequence_before_new_delivery() {
        ...
        fs::write(
            ...
            serde_json::to_string(&DeliverySnapshot {
                ...
                deliveries: vec![Delivery {
                    seq: 41,
                    chat_id: "telepathos:direct".into(),
        ...
        assert_eq!(state.queue_delivery("telepathos:direct", "new").unwrap(), 42);

        let reloaded = RelayState::default();
        reloaded.set_persist_path(&path);
        assert_eq!(
            reloaded
                .queue_delivery("telepathos:direct", "after restart")
                .unwrap(),
            43
        );
    ...
    fn consumed_queue_preserves_sequence_high_water_mark() {
        ...
        state.set_persist_path(&path);
        let seq = state.queue_delivery("telepathos:direct", "one").unwrap();
        let (picked, _) = state
            .deliveries_after(0, true, Some("telepathos:direct"), None, None)
            .unwrap();
        ...
        reloaded.set_persist_path(&path);
        assert_eq!(
            reloaded.queue_delivery("telepathos:direct", "two").unwrap(),
            seq + 1
        );
        let _ = fs::remove_file(&path);
    }
    ...
    fn persistence_creates_missing_parent_directories() {
        ...
        let state = RelayState::default();
        state.set_persist_path(&path);
        state.queue_delivery("telepathos:direct", "durable").unwrap();

        assert!(path.exists());
        let reloaded = RelayState::default();
        reloaded.set_persist_path(&path);
        assert_eq!(
            reloaded.pending_for("telepathos:direct")[0].content,
            "durable"
        );
    ...
    fn lane_id_grammar_is_enforced_for_delivery_snapshots_and_admission() {
        let invalid_lane_ids = [
            "telepathos: direct".to_string(),
            "telepathos:repo:\u{0001}control".to_string(),
            "telepathos:repo:quote\"".to_string(),
            "telepathos:repo:backslash\\".to_string(),
            format!("telepathos:repo:{}", "a".repeat(128)),
            "telepathos:repo:é".to_string(),
        ];
        ...
        let state = RelayState::default();
        state.set_persist_path(&path);
        state.queue_delivery("telepathos:direct", "stable").unwrap();
        let before = fs::read_to_string(&path).unwrap();
        for lane_id in invalid_lane_ids {
            assert!(state.queue_delivery(&lane_id, "rejected").is_err());
            assert_eq!(state.pending_count("telepathos:direct"), 1);
    ...
    fn lane_id_grammar_is_enforced_for_request_and_inbound_snapshots() {
        ...
        let state = RelayState::default();
        assert!(state
            .register_request("telepathos: direct", "tp-invalid-request")
            .is_err());
        for message_id in ["", " \t\n", "message\u{0000}control"] {
            assert!(state
                .register_request("telepathos:direct", message_id)
                .is_err());
        }
        ...
        let error = state
            .enqueue_inbound(
                ...
                &json!({
                    ...
                    "source": { "chat_id": "telepathos: direct" },
                }),
            )
            .unwrap_err();
    ...
    fn gateway_delivery_reports_persistence_failure_without_acknowledging() {
        ...
        *state.persist_path.lock().unwrap() = Some(path);
        ...
        let result = state.queue_gateway_delivery("telepathos:direct", "lost", None);
        assert!(result.is_err());
        assert!(state.pending_for("telepathos:direct").is_empty());
    ...
    fn post_rename_directory_sync_failure_poison_prevents_snapshot_rollback_or_overwrite() {
        ...
        assert_eq!(state.pending_count("telepathos:direct"), 2);
        ...
        let reloaded = RelayState::default();
        reloaded.set_persist_path(&path);
        assert_eq!(reloaded.pending_count("telepathos:direct"), 2);
...
Sec-WebSocket-Version: 13\r\n\r\n",
    ...
    fn relative_delivery_path_can_be_persisted_and_reloaded() {
        let path = PathBuf::from(format!(
            ".telepathosd-relative-pending-{}-{}.json",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
        ...
        state.set_persist_path(&path);
        state
            .queue_delivery("telepathos:direct", "relative")
            .unwrap();
    ...
    async fn superseded_connection_cannot_queue_or_acknowledge_gateway_action() {
        ...
        let (first_stop, _first_stop_rx) = tokio::sync::watch::channel(false);
        assert!(state.activate_connection(first, DEV_GATEWAY_ID.to_string(), first_tx, first_stop));
        let _ = state.register_request("telepathos:direct", "tp-old");
        ...
        let raw = r#"{"type":"outbound","requestId":"req-old","action":{"op":"send","chat_id":"telepathos:direct","content":"stale","reply_to":"tp-old"}}"#;
    ...
    fn full_delivery_queue_rejection_is_transient_and_retryable() {
        ...
        for index in 0..MAX_PENDING_DELIVERIES {
            ...
            state
                .queue_gateway_delivery("telepathos:direct", &content, None)
                .unwrap();
        }
        ...
        let request = r#"{"type":"outbound","requestId":"req-count-limited","action":{"op":"send","chat_id":"telepathos:direct","content":"overflow"}}"#;
        ...
        assert_eq!(
            state.pending_count("telepathos:direct"),
            MAX_PENDING_DELIVERIES
        );
        let pending = state.pending_for("telepathos:direct");
        ...
        assert_eq!(
            state
                .consume_lane_sequences("telepathos:direct", &[1])
                .unwrap(),
            1
        );
    ...
    fn duplicate_outbound_request_id_returns_original_result_without_second_delivery() {
        ...
        let first = handle_gateway_frame(
            &state,
            r#"{"type":"outbound","requestId":"req-duplicate","action":{"op":"send","chat_id":"telepathos:direct","content":"first"}}"#,
        )
        .unwrap();
        let retry = handle_gateway_frame(
            &state,
            r#"{"type":"outbound","requestId":"req-duplicate","action":{"op":"send","chat_id":"telepathos:direct","content":" \t\n"}}"#,
        )
        .unwrap();
    ...
    fn send_without_valid_content_is_rejected_without_mutating_queue_or_ledger() {
        ...
        handle_gateway_frame(
            &state,
            r#"{"type":"outbound","requestId":"req-seed","action":{"op":"send","chat_id":"telepathos:direct","content":"seed"}}"#,
        )
        .unwrap();
        ...
        for (request_id, action) in [
            (
                "req-missing-content",
                json!({ "op": "send", "chat_id": "telepathos:direct" }),
            ),
            (
                "req-non-string-content",
                json!({ "op": "send", "chat_id": "telepathos:direct", "content": 7 }),
            ),
            (
                "req-blank-content",
                json!({ "op": "send", "chat_id": "telepathos:direct", "content": " \t\n" }),
            ),
        ] {
    ...
    fn blank_gateway_reply_to_is_rejected_without_mutation_or_restart_failure() {
        ...
        let direct_error = state
            .queue_gateway_delivery("telepathos:direct", "must not queue", Some(" \t"))
            .unwrap_err();
        ...
        let valid: serde_json::Value = serde_json::from_str(
            &handle_gateway_frame(
                &reloaded,
                r#"{"type":"outbound","requestId":"req-valid-reply-to","action":{"op":"send","chat_id":"telepathos:direct","content":"valid reply","reply_to":"tp-valid"}}"#,
            )
            .unwrap(),
        )
        .unwrap();
        ...
        let omitted: serde_json::Value = serde_json::from_str(
            &handle_gateway_frame(
                &reloaded,
                r#"{"type":"outbound","requestId":"req-omitted-reply-to","action":{"op":"send","chat_id":"telepathos:direct","content":"omitted reply"}}"#,
            )
            .unwrap(),
        )
        .unwrap();
        ...
        let pending = reloaded.pending_for("telepathos:direct");
        assert_eq!(
            pending
                .iter()
    ...
    async fn gateway_rejects_unknown_lanes_and_accepts_api_created_lanes() {
        ...
        let rejected = handle_gateway_frame_for_connection(
            ...
            connection_id,
            DEV_GATEWAY_ID,
            r#"{"type":"outbound","requestId":"req-unknown-lane","action":{"op":"send","chat_id":"telepathos:repo:later","content":"reply"}}"#,
        )
        .await
        .unwrap();
        ...
        assert_eq!(
            ...
            "unknown lane telepathos:repo:later"
        );
    ...
    fn delivery_byte_limit_rejection_is_transient_and_retryable() {
        ...
        for _ in 0..initial_delivery_count {
            state.queue_delivery("telepathos:direct", &content).unwrap();
        }
        let snapshot_before_rejection = fs::read_to_string(&path).unwrap();
        let request = serde_json::to_string(&json!({
            "type": "outbound",
            "requestId": "req-byte-limited",
            "action": { "op": "send", "chat_id": "telepathos:direct", "content": content },
        }))
        .unwrap();
    ...
    fn delivery_content_limit_is_utf8_exact_and_rejects_without_mutation() {
        ...
        state
            .queue_delivery("telepathos:direct", &exact_ascii)
            .unwrap();
        let exact_multibyte = "🦀".repeat(MAX_DELIVERY_CONTENT_BYTES / 4);
        state
            .queue_gateway_delivery("telepathos:direct", &exact_multibyte, None)
            .unwrap();
        ...
        assert_eq!(
            state
                .queue_delivery("telepathos:direct", &oversized)
                .unwrap_err()
                .to_string(),
        ...
        assert_eq!(
            state
                .queue_gateway_delivery("telepathos:direct", &oversized, None)
                .unwrap_err()
                .to_string(),
            DELIVERY_CONTENT_TOO_LARGE_ERROR
        );
    ...
    fn oversized_gateway_delivery_returns_stable_transient_error_without_persistence() {
        ...
        let state = Arc::new(RelayState::default());
        state.set_persist_path(&path);
        state.queue_delivery("telepathos:direct", "seed").unwrap();
        let snapshot_before = fs::read(&path).unwrap();
    ...
    fn invalid_delivery_snapshot_refuses_startup_without_overwriting_it() {
        ...
        let invalid_snapshots = [
            DeliverySnapshot {
                ...
                deliveries: vec![Delivery {
                    seq: 1,
                    chat_id: "telepathos:missing".into(),
                    content: "reply".into(),
                    reply_to: None,
                arrived_at: 0,
                }],
    ...
    fn delivery_sequence_cap_preserves_durable_and_memory_state() {
        ...
        *state.next_seq.lock().unwrap() = MAX_SAFE_SEQUENCE - 1;
        ...
        assert_eq!(
            state
                .queue_delivery("telepathos:direct", "at the exact limit")
                .unwrap(),
            MAX_SAFE_SEQUENCE
        );
        ...
        let direct_error = state
            .queue_gateway_delivery("telepathos:direct", "one too many", None)
            .unwrap_err();
        assert!(direct_error
            .to_string()
    ...
    fn consumed_delivery_does_not_release_outbound_request_result() {
        ...
        let first = handle_gateway_frame(
            &state,
            r#"{"type":"outbound","requestId":"req-after-consume","action":{"op":"send","chat_id":"telepathos:direct","content":"speak once"}}"#,
        )
        .unwrap();
        ...
        let retry = handle_gateway_frame(
            &state,
            r#"{"type":"outbound","requestId":"req-after-consume","action":{"op":"send","chat_id":"telepathos:direct","content":"must not speak twice"}}"#,
        )
        .unwrap();
    ...
    fn outbound_request_result_survives_restart_and_prevents_reenqueue() {
        ...
        let first = handle_gateway_frame(
            &state,
            r#"{"type":"outbound","requestId":"req-restart","action":{"op":"send","chat_id":"telepathos:direct","content":"persisted"}}"#,
        )
        .unwrap();
        ...
        let retry = handle_gateway_frame(
            &reloaded,
            r#"{"type":"outbound","requestId":"req-restart","action":{"op":"send","chat_id":"telepathos:direct","content":"must not enqueue"}}"#,
        )
        .unwrap();
    ...
    fn abandonment_survives_restart_and_keeps_delivery_sequence_historical() {
        ...
        assert_eq!(reloaded.pending_for("telepathos:direct").len(), 2);
        assert_eq!(
            reloaded.pending_for("telepathos:direct")[0].seq,
            1,
            "the expired result's delivery remains available to the phone"
        );
        assert_eq!(reloaded.pending_for("telepathos:direct")[1].seq, 2);
        assert!(reloaded
            .retired_outbound_results
            .lock()
            .unwrap()
    ...
    fn result_retirement_reclaims_capacity_and_guards_request_id_reuse() {
        ...
        let overflow = handle_gateway_frame(
            &state,
            r#"{"type":"outbound","requestId":"send-after-ledger-full","action":{"op":"send","chat_id":"telepathos:direct","content":"must not queue"}}"#,
        )
        .unwrap();
    ...
    fn result_retirement_is_restart_safe_and_ack_retries_are_idempotent() {
        ...
        let first = handle_gateway_frame(
            &state,
            r#"{"type":"outbound","requestId":"req-restart-retire","action":{"op":"send","chat_id":"telepathos:direct","content":"speak once"}}"#,
        )
        .unwrap();
        ...
        let retry = handle_gateway_frame(
            &reloaded,
            r#"{"type":"outbound","requestId":"req-restart-retire","action":{"op":"send","chat_id":"telepathos:direct","content":"must not enqueue twice"}}"#,
        )
        .unwrap();
        let retry: serde_json::Value = serde_json::from_str(&retry).unwrap();
        assert_eq!(retry, first);
        assert_eq!(reloaded.pending_for("telepathos:direct").len(), 1);
    ...
    async fn outbound_batch_over_channel_capacity_retains_every_result() {
        ...
        let batch = (0..=RELAY_CHANNEL_CAP)
            .map(|index| {
                format!(
                    r#"{{"type":"outbound","requestId":"req-{index}","action":{{"op":"send","chat_id":"telepathos:direct","content":"reply-{index}"}}}}"#
                ) + "\n"
            })
            .collect::<String>();
    ...
    fn consuming_one_lane_preserves_other_lane_deliveries() {
        ...
        state
            ...
            .unwrap();
        state.queue_delivery("telepathos:repo:x", "x reply").unwrap();
        ...
        assert_eq!(picked.len(), 1);
        assert_eq!(latest, 2);
        assert_eq!(state.pending_count("telepathos:direct"), 0);
        assert_eq!(state.pending_count("telepathos:repo:x"), 1);
    }
    ...
    fn exact_pending_consume_preserves_unspoken_correlated_and_generic_rows() {
        let state = RelayState::default();
        let spoken = state
            .queue_delivery("telepathos:direct", "spoken generic reply")
            .unwrap();
        ...
        let unspoken_generic = state
            .queue_delivery("telepathos:direct", "unspoken generic reply")
            .unwrap();

        assert_eq!(
            state
                .consume_lane_sequences("telepathos:direct", &[spoken])
                .unwrap(),
            1
        );
        let pending = state.pending_for("telepathos:direct");
    ...
    fn oversized_delivery_boundaries_fail_without_mutating_the_durable_queue() {
        ...
        assert!(state
            .consume_lane_sequences("telepathos:direct", &[oversized])
            .is_err());
        assert!(state
            .deliveries_after(oversized, false, Some("telepathos:direct"), None, None)
            .is_err());
        assert!(state
            .deliveries_after(0, true, Some("telepathos:direct"), None, Some(oversized),)
            .is_err());
    ...
    fn bounded_reply_ack_preserves_later_reply_for_the_same_request() {
        let state = RelayState::default();
        let first = state
            .queue_gateway_delivery("telepathos:direct", "first", Some("tp-1"))
            .unwrap();
        let second = state
            .queue_gateway_delivery("telepathos:direct", "second", Some("tp-1"))
            .unwrap();

        let (spoken, _) = state
            .deliveries_after(0, true, Some("telepathos:direct"), Some("tp-1"), Some(first))
            .unwrap();
        assert_eq!(spoken.len(), 1);
        assert_eq!(spoken[0].seq, first);
        assert_eq!(state.pending_for("telepathos:direct")[0].seq, second);
    }

    #[test]
    fn uncorrelated_gateway_delivery_stays_pending() {
        let state = RelayState::default();
        let _ = state.register_request("telepathos:direct", "tp-1");
        state
            .queue_gateway_delivery("telepathos:direct", "cron", None)
            .unwrap();

        let (synchronous, _) = state
            .deliveries_after(0, false, Some("telepathos:direct"), Some("tp-1"), None)
            .unwrap();
        assert!(synchronous.is_empty());
        assert_eq!(state.pending_for("telepathos:direct")[0].content, "cron");

        let single = RelayState::default();
        let _ = single.register_request("telepathos:direct", "tp-3");
        single
            .queue_gateway_delivery("telepathos:direct", "reply", Some("tp-3"))
            .unwrap();
        let (reply, _) = single
            .deliveries_after(0, false, Some("telepathos:direct"), Some("tp-3"), None)
            .unwrap();
...
mod tests {
    ...
    fn message_event_payload_shape() {
        let ev = message_event("telepathos:direct", "direct", "hello", 7);
        assert_eq!(ev["text"], "hello");
        assert_eq!(ev["source"]["platform"], "relay");
        assert_eq!(ev["source"]["chat_id"], "telepathos:direct");
