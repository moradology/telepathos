...
/** One fully validated row from telepathosd's pending-delivery snapshot. */
...
class AudioCaptureService : Service() {
    ...
    override fun onCreate() {
        ...
        mediaSession = MediaSession(this, "Telepathos").apply {
    ...
    private fun openAfterLaneValidation(
        ...
        Thread {
            ...
            mainHandler.post {
                ...
                // telepathosd, but no lane API to query. It still receives a
    ...
    private fun fetchLaneStatus(context: PendingConsumeContext): String? {
        ...
        return try {
            val request = Request.Builder().url("$base/api/state").apply {
                context.token?.let { header("x-telepathos-token", it) }
            }.build()
    ...
    private fun fetchPendingItems(context: PendingConsumeContext): PendingFetchResult {
        val hermes = context.apiBaseUrl
            ?: return PendingFetchResult.Failed("no telepathosd URL configured", context)
        return try {
            ...
            if (hasRevision && revision == null) {
                // A configured telepathosd endpoint must provide an exact
     ...
     * telepathosd rows would otherwise orphan the prepared bridge binding.
     */
    ...
    companion object {
        ...
        private const val DEFAULT_LANE_ID = "telepathos:direct"
    ...
    fun consume(context: PendingConsumeContext) {
        ...
        try {
            ...
            val request = Request.Builder().url("$base/api/pending/consume")
                ...
                .apply { context.token?.let { header("x-telepathos-token", it) } }
                .build()
            OkHttpClient().newCall(request).execute().close()
        } catch (e: Exception) {
            Log.w("Telepathos", "inbox consume: ${e.message}")
        }
    }
}
...
private fun fetchPendingInbox(context: PendingConsumeContext): PendingInbox {
    ...
    return try {
        ...
        Log.w("Telepathos", "inbox fetch: ${e.message}")
        PendingInbox.empty()
    }
}
