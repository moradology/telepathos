...
object Foreground {
    ...
    const val ACTION_SWITCH = "dev.telepathy.SWITCH_LANE"
    const val ACTION_TALK = "dev.telepathy.TALK"
    const val ACTION_STOP = "dev.telepathy.STOP"
    ...
    private fun build(ctx: Context, text: String, lane: String?, pending: Int): NotificationCompat.Builder {
        ...
        val b = NotificationCompat.Builder(ctx, CHANNEL_ID)
            ...
            .setContentTitle(if (lane != null) "TELEPATHY · $lane" else "Telepathy")
            .setContentText(text)
            .setOngoing(true)
