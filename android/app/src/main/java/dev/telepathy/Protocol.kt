...
sealed interface ClientCommand {
    ...
    companion object {
        /** Pure mapping from AVRCP media key to command (Shokz default tap layout). */
        fun fromMediaKey(keyCode: Int): ClientCommand? = when (keyCode) {
            KeyEvent.KEYCODE_MEDIA_NEXT -> Stop
            KeyEvent.KEYCODE_MEDIA_PREVIOUS -> Repeat
            KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE -> CancelCapture
            else -> null
        }
    }
}
