export const config = {
  port: Number(process.env.TELEPATHOS_PORT ?? 8787),
  ...
  host: process.env.TELEPATHOS_HOST ?? "127.0.0.1",
  /** RMS threshold for speech detection (16-bit PCM). Tune per mic. */
  vadThreshold: Number(process.env.TELEPATHOS_VAD_THRESHOLD ?? 600),
  /** Silence duration (ms) that ends an utterance. */
  vadSilenceMs: Number(process.env.TELEPATHOS_VAD_SILENCE_MS ?? 1500),
  /** Minimum speech duration (ms) for a valid utterance. */
  vadMinSpeechMs: Number(process.env.TELEPATHOS_VAD_MIN_SPEECH_MS ?? 500),
  /** "openai" (whisper-1 API) | "local" (faster-whisper worker) | "echo" (dev stub). */
  stt: process.env.TELEPATHOS_STT ?? "echo",
  /** Agent-facing lane API (Hermes tools). localhost: only the same box needs it. */
  apiPort: Number(process.env.TELEPATHOS_API_PORT ?? 8788),
  apiHost: process.env.TELEPATHOS_API_HOST ?? "127.0.0.1",
  /** Steering agent (catches what the meta grammar misses). Unset = disabled,
   *  grammar-miss falls back to the spoken help text. */
  metaModel: process.env.TELEPATHOS_META_MODEL,
  metaBaseUrl: process.env.TELEPATHOS_META_BASE_URL ?? "https://api.openai.com/v1",
